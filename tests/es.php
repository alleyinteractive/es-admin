...
function es_admin_index_test_data() {
	...
	// Add the mapping
	$response = wp_remote_request( 'http://localhost:9200/es-admin-unit-tests/', array( 'method' => 'PUT', 'body' => '
		{
			"settings": {
				"analysis": {
					"analyzer": {
	...
	$resposne = wp_remote_post( 'http://localhost:9200/es-admin-unit-tests/_refresh' );
	travis_es_verify_response_code( $response );
}
